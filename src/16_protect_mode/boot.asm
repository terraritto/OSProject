...
%include "../modules/real/lba_chs.asm"
...
stage_4:
    ...
    mov bx, 0   ; BXにLEDの初期値を入れる

.10L:
    mov ah, 0x00
    int 0x16    ; キー入力待ち

    ; 文字としては1~3のみ許可、それ以外は終了
    cmp al, '1' ; 1より小さい場合は終了
    jb .10E

    cmp al, '3' ; 3より大きい場合は終了
    ja .10E

    mov cl, al ; CL=キー
    dec cl     ; CL--
    and cl, 0x03 ; CL &= 0x03,これでCLは0~2の値に

    mov ax, 0x0001
    shl ax, cl  ; 0x0001,0x0002,0x0004のいずれか
    xor bx, ax  ; axの立てたフラグとxorで点滅が可能
                ; XORの論理演算を参照するとわかりやすいかも
    ...
    cdecl KBC_Data_Write, 0xED  ; LEDのキーボードコマンド、0x60なのに注意
    cdecl KBC_Data_Read, .key   ; レスポンス読み出し
    
    cmp [.key], byte 0xFA       ; ACK(肯定応答)なら0xFAのはず
    jne .11F

    cdecl KBC_Data_Write, bx    ; ACKであったら書き込みOKなので、続けて書き込む
    jmp .11E
