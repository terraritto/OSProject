...
kernel:
    ...
    set_vect 0x07, int_nm           ; デバイス使用不可
    set_vect 0x0E, int_pf           ; ページフォルト
    set_vect 0x20, int_timer        ; タイマー割り込み
    set_vect 0x21, int_keyboard     ; キーボード割り込み
    set_vect 0x28, int_rtc          ; RTC割り込み
    set_vect 0x81, trap_gate_81, word 0xEF00 ; トラップゲート 0x81
    set_vect 0x82, trap_gate_82, word 0xEF00 ; トラップゲート 0x82
    ...
    ; ページングの有効化
    mov eax, CR3_BASE       ; アドレスをEAXに入れる
    mov cr3, eax            ; ページテーブルを登録！

    mov eax, cr0            ; CR0のレジスタのPEビットを1に設定していく
    or eax, (1 << 31)       ; PEを立てる
    mov cr0, eax            ; 設定！
    jmp $ + 2               ; 内部にキャッシュされた命令を破棄
